...
class PartialFC_V2(torch.nn.Module):
    ...
    def forward(
        self,
        local_embeddings: torch.Tensor,  #128
        local_labels: torch.Tensor,      #128x512
    ):
        """
        Parameters:
        ----------
...
class DistCrossEntropyFunc(torch.autograd.Function):
    ...
    def forward(ctx, logits: torch.Tensor, label: torch.Tensor):
        ...
        max_logits, _ = torch.max(logits, dim=1, keepdim=True)   #[256, 1]
        # local to global
    ...
    def backward(ctx, loss_gradient):
        ...
        logits.div_(batch_size) #torch.Size([256, 46716])
